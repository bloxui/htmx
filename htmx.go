...
func HxGet(url string) blox.Global {
	return blox.Custom("hx-get", url)
}

// HxPost sets the hx-post attribute for HTMX POST requests.
func HxPost(url string) blox.Global {
	return blox.Custom("hx-post", url)
}

// HxPut sets the hx-put attribute for HTMX PUT requests.
func HxPut(url string) blox.Global {
	return blox.Custom("hx-put", url)
}

// HxDelete sets the hx-delete attribute for HTMX DELETE requests.
func HxDelete(url string) blox.Global {
	return blox.Custom("hx-delete", url)
}

// HxPatch sets the hx-patch attribute for HTMX PATCH requests.
func HxPatch(url string) blox.Global {
	return blox.Custom("hx-patch", url)
}

// Targeting and content manipulation

// HxTarget sets the hx-target attribute to specify where to place the response.
func HxTarget(selector string) blox.Global {
	return blox.Custom("hx-target", selector)
}
...
func HxSwap(strategy string) blox.Global {
	return blox.Custom("hx-swap", strategy)
}

// HxSwapOob sets the hx-swap-oob attribute for out-of-band swaps.
func HxSwapOob(value string) blox.Global {
	return blox.Custom("hx-swap-oob", value)
}
...
func HxTrigger(event string) blox.Global {
	return blox.Custom("hx-trigger", event)
}

// Loading states and indicators

// HxIndicator sets the hx-indicator attribute to show/hide loading indicators.
func HxIndicator(selector string) blox.Global {
	return blox.Custom("hx-indicator", selector)
}

// HxDisabledElt sets the hx-disabled-elt attribute to disable elements during requests.
func HxDisabledElt(selector string) blox.Global {
	return blox.Custom("hx-disabled-elt", selector)
}
...
func HxHeaders(headers string) blox.Global {
	return blox.Custom("hx-headers", headers)
}
...
func HxVals(vals string) blox.Global {
	return blox.Custom("hx-vals", vals)
}

// HxInclude sets the hx-include attribute to include additional form elements.
func HxInclude(selector string) blox.Global {
	return blox.Custom("hx-include", selector)
}
...
func HxParams(params string) blox.Global {
	return blox.Custom("hx-params", params)
}

// Navigation and history

// HxBoost sets the hx-boost attribute to progressively enhance links and forms.
func HxBoost(enabled bool) blox.Global {
	if enabled {
		return blox.Custom("hx-boost", "true")
	}
	return blox.Custom("hx-boost", "false")
}
...
func HxPushUrl(url string) blox.Global {
	return blox.Custom("hx-push-url", url)
}

// HxReplaceUrl sets the hx-replace-url attribute to replace the current browser URL.
func HxReplaceUrl(url string) blox.Global {
	return blox.Custom("hx-replace-url", url)
}

// User interaction

// HxConfirm sets the hx-confirm attribute to show a confirmation dialog.
func HxConfirm(message string) blox.Global {
	return blox.Custom("hx-confirm", message)
}

// HxPrompt sets the hx-prompt attribute to show a prompt dialog.
func HxPrompt(message string) blox.Global {
	return blox.Custom("hx-prompt", message)
}
...
func HxExt(extensions string) blox.Global {
	return blox.Custom("hx-ext", extensions)
}

// HxSelect sets the hx-select attribute to select a subset of the response.
func HxSelect(selector string) blox.Global {
	return blox.Custom("hx-select", selector)
}

// HxSelectOob sets the hx-select-oob attribute for out-of-band content selection.
func HxSelectOob(selector string) blox.Global {
	return blox.Custom("hx-select-oob", selector)
}
...
func HxSync(strategy string) blox.Global {
	return blox.Custom("hx-sync", strategy)
}
...
func HxEncoding(encoding string) blox.Global {
	return blox.Custom("hx-encoding", encoding)
}

// Error handling and validation

// HxValidate sets the hx-validate attribute to force validation before submission.
func HxValidate() blox.Global {
	return blox.Custom("hx-validate", "true")
}
...
func HxSse(value string) blox.Global {
	return blox.Custom("hx-sse", value)
}
...
// Example: "connect:/ws"
func HxWs(value string) blox.Global {
	return blox.Custom("hx-ws", value)
}

// Preservation

// HxPreserve sets the hx-preserve attribute to preserve elements during swaps.
func HxPreserve() blox.Global {
	return blox.Custom("hx-preserve", "true")
}
...
func HxDisinherit(attrs string) blox.Global {
	return blox.Custom("hx-disinherit", attrs)
}
